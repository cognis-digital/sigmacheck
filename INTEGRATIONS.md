...
**sigmacheck** plugs into your stack through [`cognis-connect`](https://github.com/cognis-digital/cognis-connect),
...
Once `sigmacheck` emits JSON findings, pipe them straight to a destination — `--dry-run`
previews the exact request without sending:

```bash
sigmacheck ... --format json | cognis-connect emit --to sigma   # Sigma rules
sigmacheck ... --format json | cognis-connect emit --to splunk --url $URL --token $TOK   # Splunk HEC
sigmacheck ... --format json | cognis-connect emit --to elastic --url $URL --token $TOK   # Elastic _bulk
sigmacheck ... --format json | cognis-connect emit --to webhook --url $URL --token $TOK   # generic webhook
```
...
works whatever `sigmacheck` outputs:

```python
from cognis_connect import normalize, sigma
findings = [normalize(rec, source="sigmacheck") for rec in records]   # records = your JSON output
