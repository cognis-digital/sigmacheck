...
[![PyPI](https://img.shields.io/pypi/v/cognis-sigmacheck.svg)](https://pypi.org/project/cognis-sigmacheck/)
[![CI](https://github.com/cognis-digital/sigmacheck/actions/workflows/ci.yml/badge.svg)](https://github.com/cognis-digital/sigmacheck/actions)
...
SIGMACHECK exists for one job — lint and unit-test sigma detection rules against sample events — and does it without a SaaS bill or heavyweight setup.
...
```bash
sigmacheck --version
sigmacheck scan .                      # scan the current project
sigmacheck scan . --format json
sigmacheck scan . --fail-on high       # non-zero exit for CI gates
sigmacheck mcp                         # expose as an MCP server (Cognis.Studio / Claude Desktop / Cursor)
```
...
Built in the spirit of **sigma-cli**, re-framed for the Cognis approach: single-purpose, self-hostable,
MCP-native, and unified with the rest of the Suite. Missing a credit? Open a PR.

## How it fits the Cognis Neural Suite

`sigmacheck` is one of the **100+ tools** in the [Cognis Neural Suite](https://github.com/cognis-digital).
