...
- **rules/analyzers** apply the heuristics shipped in `sigmacheck/core.py`.
- **score** ranks by severity.
- **MCP server** (`sigmacheck mcp`) exposes `scan` for Cognis.Studio agents.
